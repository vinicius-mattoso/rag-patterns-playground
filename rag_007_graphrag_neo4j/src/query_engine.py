...
def build_query_chain(
    ...
    """Create a Neo4j Cypher QA chain tuned for read-oriented GraphRAG."""
...
def _extract_source_documents(
    ...
    entity_ids = sorted(
        {
            ...
            for match in re.findall(r"[A-Z]{2,}-\d+", record.payload)
        }
    )
    if not entity_ids:
        return ()
    ...
    """
    try:
        rows = graph.query(lookup_query, params={"entity_ids": entity_ids, "limit": limit})
    except Exception:
        return ()
