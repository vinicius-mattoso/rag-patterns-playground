...
def _format_node_title(labels: list[str], properties: dict) -> str:
    ...
    return f"{', '.join(labels) if labels else 'Entity'}<br>{preview}"
...
def export_graph_html(graph: Neo4jGraph, output_path: Path, relationship_limit: int) -> None:
    ...
    print(f"[rag_008] querying Neo4j for up to {relationship_limit} relationships to render html")
    rows = graph.query(GRAPH_EXPORT_QUERY, params={"limit": relationship_limit})
    print(f"[rag_008] rendering html graph with {len(rows)} relationships")
    ...
    print(f"[rag_008] graph html written to {output_path}")
